...
// Signup Endpoint
app.post('/api/signup', async (req, res) => { // This should be async
...
// Login Endpoint
app.post('/api/login', (req, res) => { // This should NOT be async
  ...
  db.get(sql, [email], async (err, user) => { // The async keyword belongs here, on the callback
    if (err) {
    ...
    return res.json({ message: 'Login successful', user: userProfile });
  });
});
